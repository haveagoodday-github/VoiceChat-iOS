...
class AliPayManager : NSObject {
    ...
    func payAlertController(_ sender: UIViewController,
        ...
        AlipaySDK.defaultService().payOrder(request, fromScheme:"alipay",callback:nil)
    }
    
    //外部用这个方法调起支付宝登录
    func login(_ withInfo:String,loginSuccess: @escaping (_ str:String) -> Void,loginFail:@escaping () -> Void){
        // sender 是调用这个方法的控制器，
        // 用于提示用户支付宝登录结果，可以根据自己需求是否要此参数。
//        self.sender = sender
        self.loginSuccessClosure = loginSuccess
        self.loginFailClosure = loginFail
        AlipaySDK.defaultService().auth_V2(withInfo:withInfo, fromScheme:"alipay", callback:nil)
    }
    ///授权回调
    func showAuth_V2Result(result: NSDictionary){
        print("ali_oauth_code 授权")
        //        9000    请求处理成功
        //        4000    系统异常
        //        6001    用户中途取消
        //        6002    网络连接出错
        ...
        let alertVc = UIAlertController.init(title: "授权结果", message: returnMsg, preferredStyle: .alert)
