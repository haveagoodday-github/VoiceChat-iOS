...
struct QQLoginView22: View {
    @State private var errorMessage: String?
    
    var body: some View {
        VStack {
            Text("QQ Login Example")
                .font(.title)
            
            Button(action: {
                QQ.shared.register(appKey: QQUtility.AppID, universalLink: QQUtility.AppLink)
                
                // Call the method to initiate the QQ authorization process
            }) {
                Text("Login with QQ")
                    .padding()
...
public class QQ: NSObject {
    ...
    /// 公钥
    private var appKey: String = QQUtility.AppKey
    
    /// 通用链接
    private var universalLink: String = QQUtility.AppLink
